#source: https://github.com/awsdocs/aws-doc-sdk-examples/blob/main/python/example_code/cognito/cognito_idp_actions.py
...
# snippet-end:[python.example_code.cognito-idp.helper.CognitoIdentityProviderWrapper.decl]
    ...
    def start_sign_in_basic(self, user_name, password):
        ...
        try:
            ...
            if err.response['Error']['Code'] == 'UserNotConfirmedException':
                response = {'success': False, 'error': err.response['Error']['Message']}
            else:
    ...
    def refresh_token(self,user_name,refresh_token):
        try:
            kwargs = {
                # 'UserPoolId': self.user_pool_id,
                'ClientId': self.client_id,
    ...
    def delete_user(self, access_token):
        try:
            self.cognito_idp_client.delete_user(AccessToken=access_token)
            return
        except ClientError as err:
            if err.response['Error']['Code'] == 'ForbiddenException':
                ...
                logger.error(
                    "Couldn't delete user. Here's why: %s: %s",
                    err.response['Error']['Code'], err.response['Error']['Message'])
            raise
    ...
    def get_user_by_token(self, access_token):
        try:
            ...
            if err.response['Error']['Code'] == 'UserNotFoundException':
                logger.error("User with given token does not exist")
            else:
                logger.error(
                    "Couldn't delete user. Here's why: %s: %s",
                    err.response['Error']['Code'], err.response['Error']['Message'])
